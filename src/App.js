...
class App extends Component {
  ...
  clickEmoji = thumbnailUrl => {
    const {imgUrl} = this.state 
    const imageValue = imagesList.filter(eachImage => eachImage.thumbnailUrl === imageUrl,)
    const {imageUrl} = imageValue[0]
    if (imageUrl === imgUrl) {
      const newImgUrl = imagesList[Math.floor(Math.random() * imagesList.length)].imageUrl
       console.log(newImagUrl)
        this.setState(prevState => ({score:prevState.score + 1, imgUrl:newImagUrl}))
    }else {
        clearInterval(this.timerId)
        this.setState({isGameRunning:true})
    }
  ...
  getFilteredProjects = () => {
    ...
    const filteredEmojis = imagesList.filter(
      eachEmoji => eachEmoji.category === activeTabId,
    )
  ...
  render() {
    const {filteredEmojis} = this.getFilteredProjects()
    const {activeTabId, score, time, isGameRunning, imgUrl} = this.state
        ...
        <div className="game-container">
          {!isGameRunning && (
              ...
              <ul className="emoji-list-container">
                    {filteredEmojis.map(eachEmoji => (
                    <EmojiCard key={eachEmoji.id} emojiCardDetails={eachEmoji} clickEmoji={this.clickEmoji}/>
                  ))}
              </ul>
          )}

            {isGameRunning && (
            <GameOverCard onClickPlayAgain={this.onClickPlayAgain}/>
            )}
